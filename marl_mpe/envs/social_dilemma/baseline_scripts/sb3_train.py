...
class VecCustomEnv(VecEnvWrapper):
    def __init__(self, venv):
        super(VecCustomEnv, self).__init__(venv)
    ...
    def step_wait(self):
        ...
        return {"observation": obs}, rewards, dones, infos

def make_env():
    return HarvestEnv(num_agents=1, return_agent_actions=False, use_collective_reward=False,
                      inequity_averse_reward=False, alpha=0.0, beta=0.0)

def main(args):
    # Config
    env_name = "harvest" # args.env_name
    num_agents = 2 # args.num_agents
    rollout_len = 1000 # args.rollout_len
    total_timesteps = 5e8 # args.total_timesteps
    ...
    # create vectorized version of env 

    # Create a single environment
    env = make_env()

    # Wrap the single environment in a DummyVecEnv to create a vectorized environment
